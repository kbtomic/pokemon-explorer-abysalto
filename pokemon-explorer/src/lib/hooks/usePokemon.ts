...
export function useAllItemsWithFallback() {
  ...
  if (detailsQuery.isError && basicQuery.data) {
    ...
    const convertedItems = basicQuery.data.results.map((item: { name: string; url: string }, index: number) => ({
      ...item,
...
// New: Hook to fetch all locations details with optimized chunking
export function useAllLocationsDetails() {
  return useQuery({
    queryKey: ['all-locations-details'],
    queryFn: () => pokeAPI.getAllLocationsDetails(),
    staleTime: 60 * 60 * 1000, // 1 hour
    gcTime: 24 * 60 * 60 * 1000, // 24 hours
    retry: 2,
    retryDelay: attemptIndex => Math.min(1000 * 2 ** attemptIndex, 30000),
