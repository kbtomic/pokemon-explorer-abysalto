...
export function StatsFilter() {
  ...
  const handleResetStat = (statName: StatName) => {
    const defaultRange = (originalStatRanges as Record<StatName, [number, number]>)[statName];
    if (defaultRange) {
  ...
  const handleClearAll = () => {
    Object.entries(originalStatRanges).forEach(([statName, defaultRange]) => {
  ...
  const activeStatsCount = Object.entries(statsFilters).filter(([statName, [min, max]]) => {
    const defaultRange = (originalStatRanges as Record<StatName, [number, number]>)[statName as StatName];
  ...
  return (
    <GenericFilter title="Stats" onClearAll={handleClearAll} selectedCount={activeStatsCount} badgeColor="bg-red-600">
      <StatsContent
        statRanges={originalStatRanges as Record<StatName, [number, number]>}
        statsFilters={statsFilters as Record<StatName, [number, number]>}
